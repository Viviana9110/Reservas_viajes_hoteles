...
const Login = () => {
  ...
  const [state, setState] = useState("login"); // "login" o "register"
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      // 👉 Guardamos token y usuario en localStorage con claves correctas
      if (res?.token) {
        localStorage.setItem("accessToken", res.token); // usamos accessToken
      }
      if (res?.user) {
