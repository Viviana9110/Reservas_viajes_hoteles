...
const MyBookings = () => {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      try {
        const res = await fetch(`${import.meta.env.VITE_API_URL}/bookings/user/${user._id}`);
  ...
  const cancelBooking = async (bookingId) => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL}/bookings/${bookingId}`, { method: "DELETE" });
  ...
  return (
    <div className="py-28 md:pb-35 md:pt-32 px-4 md:px-16 lg:px-24 xl:px-32 max-w-5xl mx-auto">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <h1 className="font-playfair text-4xl md:text-[40px] text-gray-800">Mis Reservas</h1>
        <p className="text-sm md:text-base text-gray-500 mt-2">
          Administra todas tus reservas de hotel en un solo lugar.
        </p>
      </motion.div>
      ...
      {!user ? (
        ...
        <EmptyState title="Sin reservas" description="No tienes reservas de hotel registradas." icon={assets.calenderIcon} />
      ) : (
        <motion.div variants={container} initial="hidden" animate="visible" className="space-y-4">
          <AnimatePresence>
            {bookings.map((booking) => {
              ...
              const total = booking.room?.pricePerNight ? booking.room.pricePerNight * nights : booking.totalPrice || 0;
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3">
                        <h3 className="text-lg font-semibold text-gray-800">{booking.hotel?.name}</h3>
